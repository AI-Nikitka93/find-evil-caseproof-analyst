...
- `cases/CASE-RD01/exports/correlation_summary.json`;
...
| `F006` | confirmed | Bounded registry/event correlation was performed, Event ID 1102 was preserved as a high-signal review item, and compromise status remained unconfirmed pending deeper timeline and process/account corroboration. |
...
- `Confirmed compromise or persistence on RD01` was challenged and dropped
  because the bounded registry, event, and correlation content did not
  establish compromise-level evidence.
...
| `RD01-TIME-001` | Full Plaso timeline content was not parsed in this bounded run; bounded event-derived timeline anchors were recorded in `correlation_summary.json`. | matched with limitation |
| `RD01-REG-001` | SOFTWARE Run keys and SYSTEM services were extracted and parsed into bounded registry content records. | matched with limitation |
| `RD01-EVT-001` | Security, System, and Application EVTX files were extracted and parsed into bounded event records; bounded registry/event correlation was added. | matched with limitation |
| `RD01-CORR-002` | `correlation_summary.json` records high-signal watchlist events, registry persistence surfaces, timeline anchors, and `confirmed_compromise=false`. | matched with limitation |
...
- deeper process/account/timeline corroboration beyond the bounded correlation summary;
...
consistency. The current pass also proves bounded registry/event correlation
with explicit unconfirmed compromise disposition.
...
- full Plaso timeline and deeper process/account corroboration;
- demo video;
- Devpost description;
