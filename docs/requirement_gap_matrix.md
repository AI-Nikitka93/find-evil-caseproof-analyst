...
| Accuracy validation | `verify_claim` tool, `docs/accuracy_report.md` | Present with limits | Bounded registry and event content are validated; full timeline and deeper correlation findings are not validated. | Every confirmed bounded-run finding keeps evidence support and verifier status. |
